...
export default class MasterDataPage extends React.Component<any, MasterDataPageState> {
  ...
  async refreshMasterDataAsync(): Promise<void> {
    const taskLists = (await ApiClient.get<TaskListBrief[]>(API_ENDPOINTS.TaskLists) ?? []).map((x) => {
      return {id: x.id, name: x.title};
    });
    const categories = (await ApiClient.get<Category[]>(API_ENDPOINTS.Categories) ?? []).map((x) => {
    ...
    return [
      {
        ...
        render: this.renderTabPane({
          ...
          createCallback: (title: string) => ApiClient.create(API_ENDPOINTS.TaskLists, {title: title}),
          deleteCallback: (id: number) => ApiClient.delete(`${API_ENDPOINTS.TaskLists}/${id}`),
      ...
      },
      {
        ...
        render: this.renderTabPane({
          ...
          createCallback: (name: string) => ApiClient.create(API_ENDPOINTS.Categories, {name: name}),
          deleteCallback: (id: number) => ApiClient.delete(`${API_ENDPOINTS.Categories}/${id}`)
        }),
      },
    ]
  }
